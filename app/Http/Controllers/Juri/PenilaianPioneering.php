...
class PenilaianPioneering extends Controller
{
    public function index()
    {
        ...
        $mata_lomba = MataLomba::where('nama', 'Pioneering')->first();

        // Ambil peserta yang terdaftar pada mata lomba "karikatur" dan sudah memiliki penilaian
        $pesertas = Peserta::with('penilaian_pioneering') // Eager load penilaian_karikatur
        ->where('mata_lomba_id', $mata_lomba->id)
            ->whereHas('penilaian_pioneering')  // Mengambil peserta yang sudah memiliki penilaian
            ->get();

        // Mengirim data ke view
        return view('juri.penilaian_pioneering.index', compact('pesertas'));
    }
    ...
    public function create()
    {
        ...
        $pesertas = Peserta::where('mata_lomba_id', $mata_lomba->id)
            ->whereDoesntHave('penilaian_pioneering') // Pastikan peserta belum memiliki penilaian
            ->get();

        // Mengirim data ke view
        return view('juri.penilaian_pioneering.create', compact('juris', 'pesertas', 'mata_lomba'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'kekuatan_struktur' => 'required|numeric|min:0|max:30',
            'ketepatan_kerapian' => 'required|numeric|min:0|max:25',
            'kreativitas_desain' => 'required|numeric|min:0|max:20',
            'kesesuaian_fungsi' => 'required|numeric|min:0|max:15',
