                ...
                <table id="documentTable" class="table table-bordered">
                    ...
                    <tbody>
                        @foreach($templates as $template)
                            @php
                                // Ensure `upload_dokumen` collection is not empty and get the first related `UploadDokumen` if exists
                                $uploadDokumen = $template->upload_dokumen->first() ?? null;

                                // Ensure `pembina` is related to the `UploadDokumen`
                                $pembina = $uploadDokumen ? $uploadDokumen->pembina : null;

                                // Ensure `finalisasi` is related to the `pembina`
                                $finalisasi = $pembina ? $pembina->finalisasi->first() : null;

                                // Determine the status and corresponding label
                                $status = $finalisasi->status ?? null;
                                $statusLabel = $status === 1 ? 'badge-success' : ($status === 0 ? 'badge-danger' : 'badge-info');
                                $statusText = $status === 1 ? 'Lolos Verifikasi' : ($status === 0 ? 'Tidak Lolos Verifikasi' : 'Menunggu Verifikasi');
                            @endphp
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <span class="badge badge-secondary">Tidak Ada Finalisasi</span>
                                    @endif
                                </td>
                                <td>
                                    {{ $finalisasi->keterangan ?? '-' }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
