                    ...
                    <div class="tab-pane fade show active" id="semaphore" role="tabpanel" aria-labelledby="semaphore-tab">
                        <div class="table-responsive mt-3">
                            <table class="table table-bordered text-center" id="semaphoreTable">
                                ...
                                <tbody>
                                    @foreach ($sms_questions->where('type', \App\Enums\QuestionType::SEMAPHORE->value) as $question)
                                        <tr>
                                            ...
                                            <td class="d-flex justify-content-center">
                                                <a href="{{ route('soal-sms.edit', $question->id) }}" class="btn btn-warning btn-sm mr-1" title="Edit">
                    ...
                    <div class="tab-pane fade" id="morse" role="tabpanel" aria-labelledby="morse-tab">
                        <div class="table-responsive mt-3">
                            <table class="table table-bordered text-center" id="morseTable">
                                ...
                                <tbody>
                                    @foreach ($sms_questions->where('type', \App\Enums\QuestionType::MORSE->value) as $question)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td>{{ $question->word }}</td>
                                            <td>Morse</td>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('import.soal-sms') }}" method="POST" enctype="multipart/form-data">
                    @csrf
    ...
    @foreach ($sms_questions as $question)
        <div class="modal fade" id="deleteModal{{ $question->id }}" tabindex="-1" aria-labelledby="deleteModalLabel{{ $question->id }}" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="deleteModalLabel{{ $question->id }}">Hapus Soal</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <form action="{{ route('soal-sms.destroy', $question->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
